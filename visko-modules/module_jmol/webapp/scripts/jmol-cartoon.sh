...
echo 'load ' ${infile} '; select all; wireframe off; select protein; trace 40; select protein and helix; trace off; cartoons;' > ${modpath}/scripts/jmol-script-cartoon

# executes Jmol in silent mode generating ${outfile}
java -jar ${modpath}/WEB-INF/lib/Jmol.jar -ions ${modpath}/scripts/jmol-script-cartoon -w JPEG:${outfile} > ${modpath}/output/output.txt